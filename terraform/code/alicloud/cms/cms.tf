...
resource "alicloud_cms_monitor_group_instances" "cms_monitor_group_instances" {
  ...
  instances {
    ...
    instance_name = alicloud_instance.instance.instance_name
...
# 自动获取实例类型
data "alicloud_instance_types" "instance_type_1c1g" {
  cpu_core_count = 1
  memory_size    = 1
}
...
resource "alicloud_instance" "instance" {
  ...
  instance_type   = data.alicloud_instance_types.instance_type_1c1g.instance_types[0].id
  security_groups = [
